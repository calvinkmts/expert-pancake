...
SELECT name, code, type, icon_url FROM accounting.banks
`

func (q *Queries) GetBanks(ctx context.Context) ([]AccountingBank, error) {
	rows, err := q.db.QueryContext(ctx, getBanks)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AccountingBank
	for rows.Next() {
		var i AccountingBank
		if err := rows.Scan(
			&i.Name,
			&i.Code,
			&i.Type,
